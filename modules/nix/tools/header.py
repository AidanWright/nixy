...
# creates *these* headers when treefmt is run.
# does not modify/generate content of header comment.
################################################################################
import sys
import os
...
def is_path_line(line):
    """A path line is a single-token comment (no spaces after '# ')."""
...
def repair_header(lines, path_line):
    """
    Surgically repair the header, inserting only missing pieces.
    Assumes lines[0] has already been identified as a path line.
    """
    result = list(lines)
    result[0] = path_line

    # Ensure opening SEP at index 1
    if len(result) < 2 or result[1] != SEP:
        # Find the end of any existing comment block at index 1
        comment_end = 1
    ...
    # Opening SEP is at index 1 — scan for closing SEP
    i = 2
    while i < len(result) and result[i].startswith('#') and result[i] != SEP:
        i += 1

    if i < len(result) and result[i] == SEP:
        # Complete — ensure at least one summary line exists
        if i == 2:
            result.insert(2, '# ')
        return result

    # Missing closing SEP — insert it after the summary block
    if i == 2:
        result.insert(2, '# ')
        i = 3
    result.insert(i, SEP)
