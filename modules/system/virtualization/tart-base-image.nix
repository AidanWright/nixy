      ...
      packages.tart-base-image =
        ...
        pkgs.writeShellApplication {
          ...
          text = ''
            ...
            echo "Waiting for VM guest agent..."
            deadline=$((SECONDS + 1800))
            until tart exec "$VM_NAME" true </dev/null >/dev/null 2>&1; do
              [ $SECONDS -ge $deadline ] && { echo "Error: VM guest agent did not become ready within 1800s." >&2; exit 1; }
              sleep 5
            done
            ...
            echo "Writing /etc/nix-darwin/flake.nix..."
            ...
            if [ -z "''${GH_TOKEN}" ]; then
              echo "Error: no ghcr.io token found in keychain. Store one with:" >&2
